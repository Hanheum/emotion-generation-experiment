...
class GridWorldEnv(gym.Env):
    ...
    def _get_obs(self):
        obs = np.concatenate((self._agent_location, self._target_location), axis=0)
        return obs
    ...
    def step(self, action):
        ...
        #if self.previous_distance > info['distance']:
        #    reward += 0.01
    ...
    def _render_frame(self):
        ...
        pygame.draw.rect(
            canvas, (255, 0, 0), pygame.Rect(pix_square_size*self._target_location, (pix_square_size, pix_square_size))
        )
        ...
        if self.render_mode == 'human':
            ...
            return np.transpose(np.array(pygame.surfarray.pixels3d(canvas), dtype=np.float32), axes=(1, 0, 2))
